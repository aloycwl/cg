...
contract CommFuncs is UUPSUpgradeable {
    ...
    function getCntLen(address a) external view returns(uint) {
    ...
    function cvtDec2Adr(uint a) external pure returns(address) {
        assembly {
            mstore(0x00, a)
            return(0x00, 0x20)
        }
    }
    function cvtDec2Hex(uint a) external pure returns (bytes memory b) {
        unchecked {
            uint len;
